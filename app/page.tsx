  ...
  {conversation.length === 0 ? (
    ...
    <div style={{ padding: "0.25rem 0" }}>
      {conversation.map((msg: MessageRow) => {
        ...
        `}</style>
        {/* Mobile quick-add: small floating add-lead form for phones */}
        {isMobile && (
          <div className="md:hidden" aria-hidden={false}>
            <div
              style={{
                ...
                left: '12px',
                right: '12px',
                bottom: `calc(${isDetailViewOpen ? '84px' : '12px'} + env(safe-area-inset-bottom))`,
                zIndex: 305,
                display: 'flex',
                justifyContent: 'center',
                pointerEvents: 'auto',
              ...
              {!quickAddOpen ? (
                ...
                >
                  <input
                    ...
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    style={{ flex: 1, padding: '0.5rem', borderRadius: '0.5rem', border: '1px solid #374151', background: 'transparent', color: '#fff' }}
                  />
                  <input
                    type="text"
                    placeholder="Phone"
                    value={phone}
                    onChange={(e) => setPhone(e.target.value)}
                    style={{ width: '35%', padding: '0.5rem', borderRadius: '0.5rem', border: '1px solid #374151', background: 'transparent', color: '#fff' }}
                  />
                  <button
                    type="submit"
                    disabled={loading}
                    style={{ padding: '0.5rem 0.75rem', borderRadius: '0.5rem', background: 'linear-gradient(90deg,#10b981,#34d399)', color: '#04201a', border: '1px solid rgba(255,255,255,0.04)', fontWeight: 600 }}
                  >
                    {loading ? 'Adding…' : 'Add'}
                  </button>
                  <button type="button" onClick={() => setQuickAddOpen(false)} style={{ padding: '0.35rem 0.5rem', borderRadius: '0.5rem', background: 'transparent', color: '#cbd5e1', border: '1px solid rgba(255,255,255,0.03)' }}>
                    ✕
                  </button>
                </form>
              )}
